...
module.exports = class Sender
  ...
  _completed: (task) ->
    ...
    handle.emit "complete", { handle: handle, result: task.data } unless handle.listeners("complete").length is 0
    @_removeHandle handle

  _failed: (task) ->
    handle = @_getHandle task.id
    handle.callback? task.data
    handle.emit "error", { handle: handle, error: task.data } unless handle.listeners("error").length is 0
    @_removeHandle handle
